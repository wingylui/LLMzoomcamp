...
def conversion_tracking(conversation_id, answer_js, timestamp=None):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                ...
                relevance_explanation, ans_prompt_tokens, ans_completion_tokens, ans_total_tokens, 
                eval_prompt_tokens, eval_completion_tokens, eval_total_tokens, ans_cost, eva_cost, total_cost, timestamp)
                ...
                (
                    ...
                    answer_js["answer"],
                    answer_js["model"],
                    answer_js["time_spent"],
                    answer_js["relevance"],
                    answer_js["relevent_explain"],
                    answer_js["ans_prompt_tokens"],
                    answer_js["ans_completion_tokens"],
                    answer_js["ans_total_tokens"],
                    answer_js["eva_prompt_tokens"],
                    answer_js["eva_completion_tokens"],
                    answer_js["eva_total_tokens"],
                    answer_js["ans_cost" ],
                    answer_js["eva_cost"],
                    answer_js["total_cost"],
                    timestamp
                ),
            )
        conn.commit()
...
def feedback_tracking(conversation_id, feedback, timestamp=None):
    if timestamp is None:
        timestamp = datetime.now(tz)
